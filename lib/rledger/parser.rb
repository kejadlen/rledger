...
module RLedger
  ...
  class Parser < Parslet::Parser
    ...
    rule(:ws?) { ws.maybe }
    rule(:eol) { str("\n") }
    rule(:comment) { ws? >> str(";") >> ws? >> (eol.absent? >> any).repeat.as(:comment) }
    ...
    rule(:transaction) {
      simple_date.as(:date) >>
      (ws >> ((ws? >> str(";")).absent? >> eol.absent? >> any).repeat.as(:description)).maybe >>
      comment.maybe >>
      eol >>
    ...
    rule(:posting) {
      ...
      (
        str("  ") >> ws? >>
        ((ws? >> str(";")).absent? >> eol.absent? >> any).repeat.as(:amount) >>
        comment.maybe
      ).maybe >>
      eol
    }
